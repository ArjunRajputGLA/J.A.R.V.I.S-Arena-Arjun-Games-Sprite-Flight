...
export class Sitting extends State {
    ...
    handleInput(input){
        ...
        } else if (input.includes('d') && !input.includes('s')) {
            this.game.player.setState(states.RUNNING, 1);
        }
    }
}
...
export class Running extends State { 
    ...
    handleInput(input){
        this.game.particles.unshift(new Dust(this.game, this.game.player.x + this.game.player.width * 0.5, this.game.player.y + this.game.player.height - 5)); 
        if (input.includes('s')) {
...
export class Diving extends State { 
    ...
    handleInput(input){
        ...
        const smash = document.getElementById('groundSmash');
...
export class Hit extends State { 
    ...
    handleInput(input){
        ...
        const dizziness = document.getElementById('dizzy');
        dizziness.playbackRate = 2.0;

	    dizziness.addEventListener('click', function() {
	    	if (dizziness.paused) {
	    		dizziness.play();
	    	}
	    });
